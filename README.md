...
├── public/
│   ├── favicon.svg
│   ├── apple-touch-icon.svg   Source for the iOS home-screen icon (180×180)
│   ├── apple-touch-icon.png   Generated by `npm run icons`
│   ├── og-default.svg         Source-of-truth for the default OG card (1200×630)
│   ├── og-default.png         Generated by `npm run icons`
│   └── robots.txt
├── src/
...
`public/og-default.svg` and `public/apple-touch-icon.svg` are the editable sources. Most platforms (X, LinkedIn, Facebook, Slack, iOS) **don't render SVG**, so both files are also shipped as PNG. Regenerate them whenever the SVG sources change:

```bash
npm run icons
```

That command runs `scripts/render-static-images.mjs`, which uses Astro's bundled `sharp` (via librsvg) to render `apple-touch-icon.png` at 180×180 and `og-default.png` at 1200×630. The renderer preserves gradients, paths, and text rendering identically to the SVG source — unlike `qlmanage` or macOS Preview, which produce only rough thumbnails.

`BaseHead.astro` already references `/apple-touch-icon.png` and `/og-default.png` — once you regenerate, every page picks them up automatically. Commit the PNGs alongside the SVGs so production matches what was reviewed locally.
